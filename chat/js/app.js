...
function startChat(roomId){
  ...
  var setupChatPage = function(pair){
    ...
    $messageInput.on('keydown', function(evt) {
      ...
      console.log($('#messages ul li').length+"length")
      if($('#messages ul li').length>3){
        console.log($('#messages ul li').length+"yes")
        $("#remote-video video").css("filter","blur(0px) grayscale(0%) brightness(130%) contrast(100%)")
      }
    });
  };
